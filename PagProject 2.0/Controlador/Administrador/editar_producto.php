...
<body>
	<?php
		...
		if (isset($_REQUEST['eliminar_ing'])){
			...
			$sqle = "DELETE FROM producto_has_ingredientes WHERE PRODUCTO_cod_producto = $id_prod AND Ingredientes_cod_ingrediente = $id_ing";
		...
		if (isset($_REQUEST['actualizar_prod'])){
			...
			$sqli = "UPDATE producto SET Nom_pro = '$nom', Prec_pro = '$prec', Desc_pro = '$desc', img_pro = '$img', estado_pro = $estado, CAT_Nom_cat = '$categ' WHERE cod_producto = $id";
			...
			if(isset($_POST['ingred'])){ //En caso de que se agreguen nuevos ingredientes al producto
				...
				for($i=0; $i<$number; $i++)
				{
					if(trim($_POST["ingred"][$i] != ''))
					{
						...
						$sqler = "INSERT INTO PRODUCTO_has_Ingredientes VALUES($id, $ing, $cantidad)";
				...
				<form enctype="multipart/form-data" action="#" method="post" style="margin: auto; max-width: 55%;">
					<div class="table-responsive">
						<table class="table table-bordered" id="dynamic_field">
							<tr>
								<td>Nombre</td>
								<td><input type="text" name="nombre" value="<?php echo $row['Nom_pro']; ?>" class="form-control name_list" required>
							...
							<tr>
								<td>Descripcion</td>
								<td><input type="text" name="desc" value="<?php echo $row['Desc_pro']; ?>" class="form-control name_list" required></td>
							</tr>
							<tr>
								<td>Foto</td>
							...
							<tr class="success">
								...
								<td><button type="button" name="add" id="add" class="btn btn-success">Añadir Ingrediente</button></td>
                                    ...
                                    <?php 
                                    $sql22 = "SELECT t1.cod_ingrediente, t1.Nom_ing  from ingredientes as t1 JOIN producto_has_ingredientes as t2 on (t1.cod_ingrediente = t2.Ingredientes_cod_ingrediente) WHERE t2.PRODUCTO_cod_producto = $codd";
